    ...
    >
      ...
      <div className="relative aspect-square bg-zinc-700">
        {asset.type === "video" ? (
          ...
          <img src={getThumbnail() || "/placeholder.svg"} alt={asset.name} className="w-full h-full object-cover" />
        )}
      </div>
      <div className="p-3">
...
const ProjectsAssets = () => {
  ...
  useEffect(() => {
    ...
    const fetchProjectData = async () => {
      // Solo cargar datos del proyecto cuando userFavorites esté disponible
      if (userFavorites === null) return

      setIsLoading(true)
      try {
        const projectRes = await fetch(`https://usabilidadaccesibilidad-25.onrender.com/api/projects/${projectId}`, {
        ...
        const assetsRes = await fetch(
          `https://usabilidadaccesibilidad-25.onrender.com/api/projects/${projectId}/assets`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        )
        ...
        const detailedAssets = await Promise.all(
          assetsData.map(async (assetSummary: any) => {
            try {
              const res = await fetch(`https://usabilidadaccesibilidad-25.onrender.com/api/assets/${assetSummary.id}`, {
                headers: { Authorization: `Bearer ${token}` },
              })
              if (!res.ok) throw new Error(`Failed to fetch asset ${assetSummary.id}`)
              ...
              return {
                ...
                isFavorite: userFavorites ? userFavorites.includes(detail.id) : false,
                enviroment: detail.enviroment || null,
                size: detail.size || null,
                condition: detail.condition || null,
                polycount: detail.polycount || null,
                duration: detail.duration || null,
                bitrate: detail.bitrate || null,
                resolution: detail.resolution || null,
                color_depth: detail.color_depth || null,
                frame_rate: detail.frame_rate || null,
        ...
        const validAssets = detailedAssets.filter((a) => a !== null) as Asset[]
    ...
    fetchProjectData()
  }, [navigate, location, userFavorites])
  ...
  if (isLoading) {
    return (
      <main className="p-6 mt-16 bg-[#191919] text-white min-h-screen">
        <h1 className="text-4xl font-bold mb-6 text-left">Loading...</h1>
      </main>
    )
  }
  ...
  if (error) {
    return (
      <main className="p-6 mt-16 bg-[#191919] text-white min-h-screen">
        <h1 className="text-4xl font-bold mb-6 text-left">Error</h1>
        <p>{error}</p>
      </main>
    )
  }
    ...
    <main className="p-6 mt-16 bg-[#191919] text-white min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-4xl font-bold text-left">{projectName}</h1>
